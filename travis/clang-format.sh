...
if [ "$TRAVIS_PULL_REQUEST" != "false" ]; then
    ...
    git remote add upstream https://github.com/GodotVR/godot_oculus_mobile \
...
FILES=$(git diff-tree --no-commit-id --name-only -r $RANGE | grep -v godot-cpp/ | grep -v ovr_platform_sdk/ | grep -E "\.(c|h|cpp|hpp|cc|hh|cxx|m|mm|inc|java|glsl)$")
echo "Checking files:\n$FILES"
